...
public class Ants {
	...
	public Ants(Timer timer, LocalSearch localSearch, Tsp tsp) {
		this.timer = timer;
    ...
    boolean as_flag; /* ant system */
    boolean eas_flag; /* elitist ant system */
    boolean ras_flag; /* rank-based version of ant system */
    boolean mmas_flag; /* MAX-MIN ant system */
    boolean bwas_flag; /* best-worst ant system */
    boolean acs_flag; /* ant colony system */
    ...
    int u_gb; /* every u_gb iterations update with best-so-far ant */
    ...
    void place_ant(ant_struct a, int step)
    /*
	...
	rnd = (int) (Utilities.ran01(Utilities.seed) * tsp.n); /* random number between 0 .. n-1 */

	a.tour[step] = rnd;
	a.visited[rnd] = true;
    }

    void choose_best_next(ant_struct a, int phase)
    /*
    ...
    void neighbour_choose_best_next(ant_struct a, int phase)
    /*
     * FUNCTION: chooses for an ant as the next city the one with
     * maximal value of heuristic information times pheromone
    ...
    void choose_closest_next(ant_struct a, int phase)
    /*
    ...
    void neighbour_choose_and_move_to_next(ant_struct a, int phase)
    /*
	...
	int numOfAnts = nn_ants;
	if(nn_ants>tsp.n) numOfAnts = tsp.n-1;
//	for (i = 0; i < nn_ants; i++) {
    ...
    void local_acs_pheromone_update(ant_struct a, int phase)
    /*
	...
	for (i = 0; i < num_mutations; i++) {
	    j = (int) (Utilities.ran01(Utilities.seed) * tsp.n);
	    k = (int) (Utilities.ran01(Utilities.seed) * tsp.n);
